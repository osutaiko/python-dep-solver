...
def build_encoding(repo: Dict[str, Any], python_candidates: Optional[List[str]] = None):
    ...
    package_names = sorted(repo.keys())  
...
def fitness(
    ...
    hard_pkgs = set(hard_constraints.keys()) if hard_constraints else set()
    installed_hard = 0
    for p in hard_pkgs:
        if p in ("python", "python_abi"):
            continue
        else:
            if pkg_versions.get(p) is not None:
                installed_hard += 1
    ...
    #score = (
    #    INSTALLED_REWARD * installed_hard
    #    - CONFLICT_PENALTY * conflicts
    #    - CONSTRAIN_PENALTY * constrain_conflicts
    #)
    ...
    if hard_constraints:
        for constraint_pkg, cons_list in hard_constraints.items():
            ...
            if pkg_ver is None:
                ...
                hard_constraint_penalty += 1 + dist * 1
            else:
                # 범위 조건 미충족 (>= 등)
                hard_constraint_penalty += 1
    ...
    if hard_constraints:
        ...
        for constraint_pkg, cons_list in hard_constraints.items():
            if constraint_pkg in package_names:
                pkg_idx = package_names.index(constraint_pkg) + 1 
