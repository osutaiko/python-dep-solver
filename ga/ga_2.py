...
def fitness(
    ...
    """
    목표:
      - 설치 가능한 패키지는 최대한 많이 설치 (null 최소화)
      - 그 과정에서 의존성/제약이 깨지는 패키지 수와 위반 수를 약하게 패널티
    """
    python_ver, pkg_versions = decode_individual(individual, package_names, gene_choices)

    # 버전 정보가 하나라도 있는 패키지들만 "설치 가능"으로 취급
    installable_pkgs = [pkg for pkg, versions in repo.items() if versions]

    installed_count = 0        # 설치된 패키지 수 (installable 중에서)
    broken_pkgs = set()        # 의존성/제약이 깨진 패키지 이름들
    violation_count = 0        # missing dep + conflict + constrain 위반 총합

    for pkg in installable_pkgs:
        ver = pkg_versions.get(pkg)

        # 이 패키지를 아예 설치 안 한 경우 (None) -> null, 카운트만 안 올리고 넘어감
        if ver is None:
            continue

        meta = repo.get(pkg, {}).get(ver)
        # 선택한 버전이 repo에 없으면 이 패키지는 깨진 걸로 본다
        if not meta:
            broken_pkgs.add(pkg)
            continue
    ...
    broken_count = len(broken_pkgs)

    # -------------------------
    #  스코어: null 최소화가 최우선
    # -------------------------
    # null 개수 = len(installable_pkgs) - installed_count 이고,
    # len(installable_pkgs)는 GA 내에서 상수라서
    # installed_count를 키우는 게 곧 null을 줄이는 것과 동일함.
    score = (
        1000.0 * installed_count   # 설치 가능한 애들은 일단 최대한 깐다
        - 10.0 * broken_count      # 깨진 패키지 수는 조금만 패널티
        - 1.0 * violation_count    # 세부 위반도 살짝만 패널티
    )
...
def run_ga(
    ...
    # 초기 개체군
    population = [random_individual(gene_choices) for _ in range(pop_size)]
    best_individual = None
    best_fitness = -math.inf

    for gen in range(n_generations):
        fitnesses = [fitness(ind, repo, package_names, gene_choices) for ind in population]
