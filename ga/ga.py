...
def fitness(
    ...
    """
    목적:
      - 의존성 위반(미설치, 버전 충돌, constrains 위반)을 크게 패널티
      - repo 안에 버전 정보가 있는 패키지는 웬만하면 설치하도록 보상
      - 설치된 패키지 수를 약간 보상
    """
    python_ver, pkg_versions = decode_individual(individual, package_names, gene_choices)

    # 가중치는 상황에 맞게 조정 가능
    MISSING_DEP_PENALTY = 0.5       # dependency 패키지가 아예 설치가 안 되어 있을 때
    CONFLICT_PENALTY = 3.0          # 버전 범위 조건을 만족하지 않을 때
    CONSTRAIN_PENALTY = 3.0         # constrains 위반
    INSTALLED_REWARD = 1.0          # 설치된 패키지 하나당 보상
    REQUIRED_MISSING_PENALTY = 500.0   # "설치 대상" 패키지인데 None일 때
    ...
    for pkg, ver in pkg_versions.items():
        ...
        for dep_pkg, cons_list in depends.items():
            # python / python_abi 는 가상 패키지: python_ver로 검사
            if dep_pkg in ("python", "python_abi"):
                dep_ver = python_ver
            else:
                # repo에 없는 패키지에 대한 의존성은 여기서 스킵하거나,
                # 패널티를 주도록 바꿀 수 있음.
                if dep_pkg not in pkg_versions:
                    # 그냥 무시 (실제 실험할 땐 원하는 정책으로 수정)
                    missing_dep += 1
                    continue
